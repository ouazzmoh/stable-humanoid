...
from pink import solve_ik, build_ik
from loop_rate_limiters import RateLimiter
from utils import get_control_points
import numpy as np
import pink
from utils import store_qp_in_file
...
def move_foot(configuration : pink.Configuration,
              ...
              time: float=0.0,
              iteration: int=0,
              visualizer: Optional[pin.visualize.MeshcatVisualizer]=None,
              display:Optional[bool]=True,
              store_qp: Optional[bool]=False,
              file=None,
              **kwargs):
    """
    ...
    Args:
        ...
        visualizer: Meshcat visualizer for visualization (default: None).
    ...
    while t <= 1:
            ...
            if visualizer:
                ...
                visualizer.viewer["r_ankle"].set_transform(configuration.get_transform_frame_to_world(foot_task.body).np)
            # Compute velocity and integrate it into next configuration
            if store_qp and file:
                configuration.check_limits()
                problem = build_ik(configuration, tasks, dt)
                qp_data = {key : value for key, value in problem.__dict__.items() if value is not None}
                store_qp_in_file(file, time + t, f"ik_qp_{(iteration + i):04}", "ik", **qp_data)
                i += 1
            velocity = solve_ik(configuration, tasks, dt, solver=solver)
            configuration.integrate_inplace(velocity, dt)
            # Visualize result at fixed FPS
            if display and visualizer:
