...
def move_foot(
    ...
    **kwargs,
    ...
    while t <= 1:
        ...
        right_foot_target.rotation = pin.AngleAxis(
            orientation[round(t * frequency)], np.array([0, 0, 1])
        ).toRotationMatrix()
        ...
        if store_qp and file:
            ...
            store_qp_in_file(
                file, time + t, f"ik_qp_{(iteration + i):04}", "ik", **qp_data
            )
            i += 1
...
def get_com_positions(
    src: np.ndarray, dst: np.ndarray, frequency: Optional[float] = 50.0
):
    """
